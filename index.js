...
// --- DB CONNECTION ---
// ENSURE "Allow Access From Anywhere" (0.0.0.0/0) is set in MongoDB Atlas Network Access!
...
const StudentSchema = new mongoose.Schema({
    name: String, email: { type: String, unique: true }, password: String, joinedAt: { type: Date, default: Date.now }, testAttempts: { type: Map, of: Number, default: {} }
});
...
// Announcement & Blogs
app.get('/api/announcement', async (req, res) => { const ann = await Config.findOne({ type: 'announcement' }); res.json({ text: ann ? ann.content : "Welcome!" }); });
...
app.post('/api/material/unlock', async (req, res) => {
    ...
    if(file && file.accessCode === req.body.code) res.json({ success: true, link: file.link }); else res.json({ success: false });
});
app.get('/api/results/offline', async (req, res) => {
    const results = await OfflineResult.find();
    res.json(results.map(r => ({ _id: r._id, title: r.title, date: r.date, records: r.records.map(rec => ({ studentName: rec.studentName, marks: rec.marks, rank: rec.rank, _id: rec._id })) })));
});
app.post('/api/results/unlock-copy', async (req, res) => {
    const { resultId, recordId, password } = req.body; const result = await OfflineResult.findById(resultId); const record = result.records.id(recordId);
    if(record && record.accessCode === password) res.json({ success: true, link: record.copyLink }); else res.json({ success: false });
});

// Test Engine
app.get('/api/tests', async (req, res) => res.json(await Test.find({}, 'title duration date')));
app.post('/api/test/start', async (req, res) => {
    const { id, code, studentEmail } = req.body; const student = await Student.findOne({ email: studentEmail });
    if(!student) return res.json({ success: false, message: "Login first." });
    const attempts = student.testAttempts.get(id) || 0; if (attempts >= 3) return res.json({ success: false, message: "Max 3 attempts reached." });
    const test = await Test.findById(id);
    if(test && test.accessCode === code) {
        student.testAttempts.set(id, attempts + 1); await student.save();
