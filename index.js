...
// Updated Config for Multiple Announcements
...
app.post('/api/admin/announcement/delete', async (req, res) => {
    const c = await Config.findOne({ type: 'announce_list' });
    if(c) {
        c.list = c.list.filter(t => t !== req.body.text);
        await c.save();
    }
    res.json({ success: true });
});
...
// Exam Engine
app.get('/api/tests', async (req, res) => res.json(await Test.find({}, 'title duration date accessCode'))); // Added accessCode to field list
...
app.post('/api/test/start', async (req, res) => {
    ...
    // Logic: If test has no password, allow instantly. If has password, check it.
...
app.post('/api/test/submit', async (req, res) => {
    try {
        ...
        for(let topic in topicStats) {
            let p = topicStats[topic].total > 0 ? (topicStats[topic].obtained / topicStats[topic].total) * 100 : 0;
