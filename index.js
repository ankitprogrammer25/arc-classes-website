...
// --- DB CONNECTION ---
// Make sure you did the "Allow Access From Anywhere" step in MongoDB Atlas!
...
const StudentSchema = new mongoose.Schema({
    ...
    testAttempts: { type: Map, of: Number, default: {} } // Tracks attempts
});
...
app.post('/api/register', async (req, res) => {
    ...
    } catch (e) { res.json({ success: false, message: "Error registering" }); }
});
...
app.post('/api/test/start', async (req, res) => {
    ...
    if(!student) return res.json({ success: false, message: "Student not found. Please Login." });

    const attempts = student.testAttempts.get(id) || 0;
    if (attempts >= 3) return res.json({ success: false, message: "Maximum 3 attempts reached." });
...
app.post('/api/test/submit', async (req, res) => {
    ...
    test.questions.forEach((q, i) => {
        ...
        else if(answers[i] !== null && answers[i] !== undefined) score -= q.negative;
    });

    const percentage = total > 0 ? (score / total) * 100 : 0;
    let feedback = "";
    if (percentage < 40) feedback = "Don't worry. Review the notes and try again.";
    else if (percentage < 80) feedback = "Good job! A little revision will make you perfect.";
    else feedback = "Outstanding! You have mastered this topic.";
